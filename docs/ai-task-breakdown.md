...
사용자가 AI 결과에서 채택할 단계를 고르고 시작 시간을 정한 뒤 한 번에 저장한다.
요청 전체가 하나의 트랜잭션으로 처리되므로 검증에 실패한 일정이 있으면 일부만 저장되지 않는다.
...

{
  "schedules": [
    {
      "title": "발표 목차 정리",
      "description": "핵심 메시지와 순서를 정한다.",
      "startAt": "2026-07-21T19:00:00",
      "endAt": "2026-07-21T19:20:00",
      "color": "#f59e0b",
      "repeatType": "NONE"
    }
  ]
...
한 요청에는 최대 20개 일정을 저장할 수 있다. 모바일은 선택된 AI 단계를 예상 시간 순서대로 이어 붙여 요청하며, 에너지 수준에 따라 색상을 구분한다.
...
5. 사용자가 확인한 단계만 배치 API로 일정에 저장한다. AI 결과를 자동으로 일정에 쓰지 않아 사용자 통제와 잘못된 생성 결과의 영향을 제한한다.
